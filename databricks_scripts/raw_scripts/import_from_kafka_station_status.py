...
from pyspark.sql.functions import col, explode, split, struct, array, lit, udf, from_json, window, to_date, \
    to_timestamp, current_date, array_max, to_timestamp, max, avg, round, count, row_number
from pyspark.sql.types import StringType, IntegerType, TimestampType, StructType, StructField, ArrayType, DateType, \
    BooleanType, BinaryType, LongType
...
schemaRegistryApiKey = dbutils.secrets.get(scope="de_project", key="ConfluentSchemaRegistryKey")
schemaRegistrySecret = dbutils.secrets.get(scope="de_project", key="ConfluentSchemaRegistrySecret")
...
kafka_df = (
    ...
    .select(from_json(col("value"), schema).alias("value"), col("topic"), col("partition"), col("offset"),
            col("timestamp"))
...
# Explode the stations array and rename column
...
explode_df = station_df \
    ...
    .groupby(window("timestamp", "10 minutes"), "station_id", "legacy_id", "num_ebikes_available",
             "num_bikes_available", "num_bikes_disabled", "is_installed", "is_returning", "is_renting",
             "eightd_has_available_keys", "num_docks_available", "num_docks_disabled", "num_scooters_available",
             "num_scooters_unavailable", "last_reported", "batch_run_date") \
    ...
    .select(col("window.start").alias("window_start"), "station_id", "legacy_id", "num_ebikes_available",
            "num_bikes_available", "num_bikes_disabled", "is_installed", "is_returning", "is_renting",
            "eightd_has_available_keys", "num_docks_available", "num_docks_disabled", "num_scooters_available",
            "num_scooters_unavailable", to_timestamp("max_last_reported").alias("max_last_reported"),
            to_timestamp("max_last_updated").alias("max_last_updated"), "batch_run_date"
            )
...
# test Connection to Snowflake and read data from the table
...
df = spark.read.format(SNOWFLAKE_SOURCE_NAME) \
    .options(**sfOptions) \
    .option("query", "SELECT * FROM CITIBIKE.RAW.BIKE_STATION_STATUS LIMIT 10") \
    .load()
